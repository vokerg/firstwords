...
.controller('MainController', ['$http', '$scope', '$timeout', '$location', 'questionsService', 
	...
	this.loadNextQuestion = function(correctPrevious) {
		...
		console.log('wordCount=' + $scope.wordCount + ', correctAnswers=' + questionsService.correctAnswers);
		$http.get('/questions/Home/' + $scope.wordCount + '/' + questionsService.correctAnswers).success(function(data) {
			if ($scope.wordCount != data.questions.length) {
				questionsService.correctAnswers = -1;
			}
			$scope.wordCount = data.questions.length;					
